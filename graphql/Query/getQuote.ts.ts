...
export const getCreditReport: QueryResolvers["getCreditReport"] = async (
  ...
  const { auto, flood, homeowners, mortgage, propertyId, creditId } = args;

  const property: EstatedProperty = await context.parcelClient.getUserProperty(
    id,
    userId
  );
  const credit: EquifaxCreditReportParent =
    await context.parcelClient.getUserCreditReport(id, userId);
